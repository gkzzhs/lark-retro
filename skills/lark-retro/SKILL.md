...
name: lark-retro
version: 2.6.7
description: "Sprint/周期回顾工作流：自动从日历（含会议纪要/会议录制记录）、OKR、审批、任务、消息、文档、画板中采集数据，AI 生成结构化报告，并追踪改进项闭环。支持 OKR 对齐与进展记录分析、审批阻塞信号补强、历史文档权限申请补救、自有云文档搜索增强、@提及消息过滤、行动项自动关闭、任务列表自定义分组、历史报告对比、Bitable 归档、记录分享链接、报告空间初始化、报告快捷方式归档、搜索/预约/更新下期回顾会议室。当用户需要做回顾、复盘、retrospective、周报总结时使用。"
metadata:
  requires:
    bins: ["lark-cli"]
...
# 可选增强（lark-cli v1.0.21+：OKR 进展记录只读分析）
lark-cli auth login --scope "okr:okr.progress:readonly"
...
| **高级版** | + 消息分析 + 文档搜索 + 知识库归档 + 会议纪要/会议记录分析 + OKR 对齐/进展记录 + 审批阻塞信号 | + `--scope "search:message search:docs:read minutes:minute:read vc:record:readonly okr:okr.period:readonly okr:okr.content:readonly okr:okr.progress:readonly approval:instance:read approval:task:read"` |
...
Step 1: 确定报告模式 & 时间范围（推断用户意图 → 用系统命令计算 start/end）
    ...
    ├─► Step 2c: okr +cycle-list/+cycle-detail/+progress-list ─► OKR 目标/KR/进展对齐（可选, v1.0.21）
...
Step 6: AI 分析 & 生成结构化回顾报告
    ...
    ├─► Step 10: calendar +room-find ────────► 预约下期回顾会议室（⚠️ 用户确认, v1.0.21 多名称）
    ├─► Step 10a: calendar events search_event ─► 查找已有下期回顾日程（可选, v1.0.21）
...
## Step 2c: 采集 OKR 目标/KR/进展记录（可选 — v1.0.14+/v1.0.21+）
...
# v1.0.21+：对关键目标读取进展记录，判断本周期是否有真实推进
lark-cli okr +progress-list \
  --target-id "<objective_or_key_result_id>" \
  --target-type objective

# 如需读取单条进展详情
lark-cli okr +progress-get --progress-id "<progress_id>"
```
...
>
> **实测边界（v1.0.21）**：`okr +progress-list` / `+progress-get` 需要 `okr:okr.progress:readonly`。当前测试账号缺少该 scope，真实命令返回 `missing_scope`，因此本步骤必须可降级。`okr +progress-create` 需要 `okr:okr.progress:writeonly`，`+progress-delete` 是高风险写操作并要求 `--yes`。
>
> **安全边界**：lark-retro 默认只读取 OKR 和进展记录，不创建、不更新、不删除目标、关键结果或进展。只有当用户明确说"把本次复盘结论写入 OKR 进展"并确认 target/progress 内容后，才允许单独执行 `+progress-create` 或 `+progress-update`；默认禁止 `+progress-delete`。

**从 OKR 数据中提取：**
- 周期 ID、周期起止时间、周期状态、周期得分
- Objective 标题、权重、得分、截止时间
- Key Result 标题、权重、得分、截止时间
- OKR 进展记录：本周期更新频率、进展百分比、状态（normal/overdue/done）、是否长期无更新
...
## Step 10: 预约下期回顾会议室（可选 — ⚠️ 需用户确认, v1.0.8+/v1.0.21+）
...
# v1.0.21+：如果用户点名多个候选会议室，可一次传入逗号分隔名称
lark-cli calendar +room-find --slot "2026-04-17T16:00:00+08:00~2026-04-17T17:00:00+08:00" \
  --room-name "01,02,03" --min-capacity 5
```
...
> 3. 列出会议室供用户选择。
> 4. 确认后调用 `lark-cli calendar +create --format table` 创建下期回顾日程并锁定会议室，表格输出更适合现场确认。
>
> **实测结果（v1.0.21）**：`calendar +room-find --room-name "01,02,03"` 已真实执行，返回 `ok: true`，dry-run 中 body 包含 `room_name: "01,02,03"`。当前账号没有命中具体会议室候选时只返回 `time_slots`，不要误判为失败。

## Step 10a: 搜索已有下期回顾日程（可选 — v1.0.21+）

在创建新会议前，先搜索是否已经存在下期回顾，避免重复预约：

```bash
lark-cli calendar events search_event \
  --params '{"calendar_id":"primary","page_size":10}' \
  --data '{"query":"回顾","filter":{"time_range":{"start_time":"2026-04-29T00:00:00+08:00","end_time":"2026-05-06T23:59:59+08:00"}}}'
```

> **实测边界（v1.0.21）**：真实创建临时日程后立即 `search_event` 返回 0，说明日程搜索同样可能有索引延迟。搜索不到不代表没有日程；可回退到 `calendar +agenda` 在同一时间窗内按标题筛选。
...
| `calendar events search_event` | `--scope "calendar:calendar.event:read"` | 否（搜索已有回顾日程） |
...
| `okr +progress-list` / `+progress-get` | `--scope "okr:okr.progress:readonly"` | 否（OKR 进展只读分析） |
| `okr +progress-create` / `+progress-update` | `--scope "okr:okr.progress:writeonly"` | 否（写操作，需明确确认） |
| `okr +progress-delete` | `--scope "okr:okr.progress:writeonly"` + `--yes` | 否（高风险，默认禁用） |
...
| `missing_scope: okr:okr.progress:readonly` | 未授权 OKR 进展记录读取 | 保留目标/KR 信息，跳过进展记录分析 |
| `missing required scope(s): okr:okr.progress:writeonly` | 尝试创建/更新 OKR 进展但未授权 | 默认不要求补授权；只有用户明确要写入 OKR 进展时再提示 |
...
| `calendar events search_event` 返回 0 | 日程搜索未命中或索引延迟 | 回退到 `calendar +agenda` 同时间窗筛选标题，不要直接创建重复会议 |
...
- **⚠️ OKR 进展默认只读**：`okr +progress-list` / `+progress-get` 只用于判断推进状态；`+progress-create` / `+progress-update` 必须经用户明确确认，`+progress-delete` 默认禁用。
...
- [lark-calendar](../lark-calendar/SKILL.md) — `+agenda`, `+room-find`, `events search_event`, `+update`
- [lark-vc](../lark-vc/SKILL.md) — `+search`, `+notes`
- [lark-okr](../lark-okr/SKILL.md) — `+cycle-list`, `+cycle-detail`, `+progress-list`, `+progress-get`
