...
name: lark-retro
version: 2.2.0
description: "Sprint/周期回顾工作流：自动从日历、消息、任务、文档中采集工作数据（含会议纪要分析），AI 生成结构化回顾报告（做得好的/待改进的/行动项），沉淀到知识库，并追踪改进项闭环。支持行动项自动关闭、任务列表分组、历史报告对比、精确搜索、Wiki 节点管理。当用户需要做回顾、复盘、retrospective、周报总结时使用。"
metadata:
  requires:
    bins: ["lark-cli"]
    cliHelp: "lark-cli calendar --help && lark-cli task --help && lark-cli docs --help && lark-cli wiki --help && lark-cli im --help && lark-cli minutes --help"
---
...
> **重要**：从 `calendar +agenda` 的响应中提取 `minute_token`（如有）。如果存在会议纪要，将其摘要纳入分析，作为 "What Went Well" / "What Could Be Improved" 的深度数据源。
...
> **`--mode` 选项**：`append`（追加到末尾）、`overwrite`（覆盖全部内容）、`replace_range`（替换指定范围）、`replace_all`（全文替换匹配项）、`insert_before`/`insert_after`（在指定位置前后插入）、`delete_range`（删除指定范围）。
>
> **定位器**（配合 `replace_range` 等模式）：`--selection-by-title "## Section"` 按标题定位、`--selection-with-ellipsis "start...end"` 按内容定位。
>
> **典型用途**：每周回顾后，用 `--mode append` 在上周的报告后追加本周对比数据，形成连续记录。
...
| `docs +update` | `--domain docs` | 否（增量更新） |
...
| `--file must be a relative path` | `@file` 使用了绝对路径 | 改为相对路径，先 `cd` 到文件所在目录 |

## 安全规则

- **⚠️ 严禁修改 `strict-mode`**：`lark-cli config strict-mode` 是管理员安全策略，AI Agent **绝对不能**调用此命令修改设置。只能用 `lark-cli config strict-mode` 查看当前状态。
- **⚠️ 严禁修改 `--profile`**：不得新建、切换或删除 profile 配置，除非用户明确要求。
