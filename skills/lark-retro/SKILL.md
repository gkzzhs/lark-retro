...
name: lark-retro
version: 2.5.0
description: "Sprint/周期回顾工作流：自动从日历（含会议纪要/会议录制记录）、任务、消息、文档、画板中采集数据，AI 生成结构化报告，并追踪改进项闭环。支持行动项自动关闭、任务列表自定义分组、历史报告对比、Bitable 归档、报告快捷方式归档、预约下期回顾会议室。当用户需要做回顾、复盘、retrospective、周报总结时使用。"
metadata:
  requires:
    bins: ["lark-cli"]
    cliHelp: "lark-cli calendar --help && lark-cli task --help && lark-cli docs --help && lark-cli wiki --help && lark-cli drive --help && lark-cli im --help && lark-cli minutes --help && lark-cli vc --help && lark-cli base --help && lark-cli whiteboard --help"
---
...
| **高级版** | + 消息分析 + 文档搜索 + 知识库归档 + 会议纪要/会议记录分析 | + `--scope "search:message search:docs:read minutes:minute:read vc:record:readonly"` |
...
Step 6: AI 分析 & 生成结构化回顾报告
    ...
    ├─► Step 7c: drive +create-shortcut ───► 报告快捷方式归档（可选, v1.0.10）
...
8. **Emoji 视觉增强**：在标题、表格指标、每条洞察和行动项前添加语义相关的 emoji。
...
> **v1.0.7 权限优化**：应用创建的文档会自动授予用户编辑权限，用户可直接点击 URL 进一步完善。
...
### Step 7c: 报告标题修正与快捷方式归档（可选 — v1.0.10+）
...
> **实测边界（v1.0.10）**：`drive files patch` 修改 docx 标题可用；`drive +create-shortcut` 需要有效的目标 `folder_token`，`--folder-token ""` 会被 CLI 拒绝。创建快捷方式后，`drive files list --params '{"folder_token":"<target_folder_token>"}'` 会返回 `type: "shortcut"` 和 `shortcut_info.target_token`。
>
> **权限**：快捷方式需要 `space:document:shortcut`，读取文件夹清单验证需要 `space:document:retrieve`，创建临时/目标文件夹需要 `space:folder:create`；标题修改 docx 需要 `docx:document:write_only`。
>
...
| `drive +create-shortcut` | `--scope "space:document:shortcut"`；验证文件夹需 `space:document:retrieve`；创建文件夹需 `space:folder:create` | 否（报告快捷方式归档） |
...
- [lark-calendar](../lark-calendar/SKILL.md) — `+agenda`, `+room-find`
- [lark-vc](../lark-vc/SKILL.md) — `+search`, `+notes`
- [lark-task](../lark-task/SKILL.md) — `+get-my-tasks`, `+create`, `+complete`, `+comment`
- [lark-doc](../lark-doc/SKILL.md) — `+create`, `+search`, `+update`
- [lark-wiki](../lark-wiki/SKILL.md) — `+node-create`
- [lark-im](../lark-im/SKILL.md) — `+messages-search`, `+messages-send`, `+chat-messages-list`
- [lark-drive](../lark-drive/SKILL.md) — `+export`, `+create-shortcut`, `files patch`
- [lark-base](../lark-base/SKILL.md) — `+record-batch-create`
