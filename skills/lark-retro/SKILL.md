...
name: lark-retro
version: 2.3.0
description: "Sprint/周期回顾工作流：自动从日历（含会议纪要）、任务、消息、文档、画板中采集数据，AI 生成结构化报告，并追踪改进项闭环。支持行动项自动关闭、任务列表分组、历史报告对比、Bitable 归档、预约下期回顾会议室。当用户需要做回顾、复盘、retrospective、周报总结时使用。"
metadata:
  requires:
    bins: ["lark-cli"]
    cliHelp: "lark-cli calendar --help && lark-cli task --help && lark-cli docs --help && lark-cli wiki --help && lark-cli im --help && lark-cli minutes --help && lark-cli base --help && lark-cli whiteboard --help"
---
...
| **高级版** | + 消息分析 + 文档搜索 + 知识库归档 + 会议纪要分析 | + `--scope "search:message search:docs:read minutes:minute:read"` |
