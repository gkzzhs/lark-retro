...
name: lark-retro
version: 2.1.0
description: "Sprint/周期回顾工作流：自动从日历、消息、任务、文档中采集工作数据，AI 生成结构化回顾报告（做得好的/待改进的/行动项），沉淀到知识库，并追踪改进项闭环。支持行动项自动关闭、任务列表分组、历史报告对比、文档增量更新。当用户需要做回顾、复盘、retrospective、周报总结时使用。"
metadata:
  requires:
    bins: ["lark-cli"]
...
### 推荐：使用 `@file` 模式（v1.0.5+）

对于长报告，**先将 markdown 写入本地临时文件，再用 `@file` 引用**，避免 shell 转义问题：

```bash
# 1. 将报告写入本地临时文件（当前目录下的相对路径）
# （由 AI Agent 的 write 工具完成）
