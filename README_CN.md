...
**基于飞书 CLI 的 AI 回顾工作流**
一句话触发周期回顾：自动读取日历、任务、消息、文档数据，生成结构化 Sprint Retro / 周报 / 工作复盘，并可沉淀到知识库、创建行动项、发送通知。
...
- 回顾靠记忆，容易只剩主观感受
- 数据散落在日历、任务、群聊、文档里，很难拼起来看
- 上次定下的 Action Items，下一次回顾时往往已经没人追踪
- 会议花了 1 小时，真正形成洞察和改进动作的时间只有几分钟

`lark-retro` 的目标，就是把回顾从"聊一聊"变成"基于飞书真实工作数据的自动化闭环"。
...
> ⚠️ 第 2 步必须先于第 3 步执行。`lark-retro` 依赖官方 `lark-shared` Skill 处理认证和权限。
>
